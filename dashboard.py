...
class IntegratedCrowdAnalyticsDashboard:
        ...
        try:
            if len(counts) >= 20:
                ...
                results['anova_test'] = {
                    ...
                    'hypothesis': 'H0: No difference between older and newer half of the timeline'
                }
            else:
                results['anova_test'] = {
                    'test': 'One-Way ANOVA Test',
    ...
    def display_real_time_metrics(self, data, stats_dict, risk_analysis):
        ...
        # Add Space Utilization Gauge below metrics
        st.markdown("<br>", unsafe_allow_html=True)
        gauge_col1, gauge_col2, gauge_col3 = st.columns([1, 2, 1])
        with gauge_col2:
            max_capacity = data.get('alert_thresholds', {}).get('high', 30) * 1.5
            current_count = data.get('current_count', 0)
            capacity_pct = min(100, (current_count / max_capacity) * 100) if max_capacity > 0 else 0
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge = {
                    'axis': {'range': [0, 100]},
                    'bar': {'color': "#1f77b4" if capacity_pct < 80 else "#ff1744"},
                    'steps': [
                        {'range': [0, 40], 'color': "#e8f5e8"},
                        {'range': [40, 80], 'color': "#fffde7"},
    ...
    def display_data_export(self, data):
        ...
        with col1:
            ...
            st.subheader("⚠️ Incident Alert Log")
            if data['people_history'] and len(data['people_history']) > 0:
                high_alerts = [count for count in data['people_history'] if count > 30]
                if high_alerts:
                    alert_df = pd.DataFrame({
                        'Severity': ['CRITICAL'] * len(high_alerts),
                        'Crowd Size Logged': high_alerts
                    })
                    st.dataframe(alert_df, use_container_width=True, height=200)
                else:
                    st.success("No critical alerts logged recently.")
            else:
...
if __name__ == "__main__":
    ...
    # Auto-refresh without while-loop blocking
    time.sleep(3)
    st.rerun()